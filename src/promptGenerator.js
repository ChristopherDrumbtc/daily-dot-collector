import { AREAS, ACTIVITY_TAGS, dk, fmtDate } from './constants';
...
export function generatePrompt(data, date, entry, avg, filled, last30, areaRank, weakest) {
  ...
  p += `IL MIO PROFILO\n`;
  p += `═══════════════════════════════════════\n\n`;
  p += `Sto usando il Daily Dot Collector, un sistema basato sui Principi di Ray Dalio per misurare la mia crescita quotidiana su 10 aree. Ti passo i miei dati così puoi organizzarmi la giornata in modo ottimale.\n\n`;
  ...
  if (filled.length > 0) {
    ...
    const acts = ACTIVITY_TAGS.filter(t => entry.activities?.[t.id]);
